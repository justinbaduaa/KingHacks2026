#!/usr/bin/env python3
"""
Interactive Slack OAuth tester (user token) using backend redirect.

Usage:
  python test_slack_oauth_code.py
"""
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Slack OAuth code exchange test")
    ...
    start_url = f"{api_url}integrations/slack/start"
    headers = {"Authorization": f"Bearer {token}", "Content-Type": "application/json"}
    response = requests.post(start_url, headers=headers, data="{}", timeout=30)
    if response.status_code >= 300:
        print("ERROR: Failed to start Slack OAuth", file=sys.stderr)
        print(response.text)
        return 2
    ...
    print("Open this URL to authenticate with Slack:")
    print(auth_url)
    ...
    token_url = f"{api_url}integrations/slack/token"
    ...
    print("Timed out waiting for Slack connection.")
    return 2
