...
from lib.microsoft_oauth import MicrosoftOAuthError, exchange_code_for_tokens
from lib.microsoft_store import store_microsoft_tokens
...
def handler(event, context):
    ...
    state_item = consume_oauth_state("microsoft", state)
    if not state_item:
    ...
    try:
        ...
        store_microsoft_tokens(user_id, payload)
    except MicrosoftOAuthError as exc:
        return _html_response(400, f"Microsoft OAuth failed: {exc}")
    except Exception:
        return _html_response(502, "Microsoft OAuth failed.")

    return _html_response(200, "Microsoft connected successfully.")
