#!/usr/bin/env python3
"""
Test script for Slack execution via complete_node or execute_node.

Usage:
  python test_slack_execute.py --message "Hello" --channel-name general
  python test_slack_execute.py --message "Hi" --recipient-name Evan
"""
...
def build_slack_node(node_id, message, channel_name=None, channel_id=None, recipient_name=None, recipient_id=None):
    now_iso = datetime.now(timezone.utc).isoformat()
    slack_payload = {
        "message": message,
    }
    if channel_name:
        slack_payload["channel_name"] = channel_name
    if channel_id:
        slack_payload["channel_id"] = channel_id
    if recipient_name:
        slack_payload["recipient_name"] = recipient_name
    if recipient_id:
        slack_payload["recipient_id"] = recipient_id
    ...
    return {
        ...
        "status": "active",
        "confidence": 0.9,
        "evidence": [{"quote": "Test Slack message"}],
...
def main():
    parser = argparse.ArgumentParser(description="Test Slack execution")
